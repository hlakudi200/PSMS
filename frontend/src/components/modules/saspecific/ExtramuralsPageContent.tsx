...
function ActivitiesTab() {
  ...
  const { extramuralActivities, totalCount, isPending, isError } = useExtramuralActivityState();
  ...
  const columns: ColumnConfig<IExtramuralActivityList>[] = [
    { key: 'activityName', title: 'Activity Name', dataIndex: 'activityName', sortable: true },
    {
      key: 'category', title: 'Category', dataIndex: 'category', width: 110,
    ...
    {
      key: 'activityType', title: 'Type', dataIndex: 'activityType', width: 100,
      filterable: true, filterType: 'enum',
      filterOptions: [
    ...
    {
      key: 'dayOfWeek', title: 'Day', dataIndex: 'dayOfWeek', width: 100, hideOnMobile: true,
      renderType: 'custom',
      render: (value: number | undefined) => value != null ? dayOfWeekMap[value] ?? '-' : '-',
    },
    { key: 'coachName', title: 'Coach', dataIndex: 'coachName', hideOnMobile: true },
    {
      key: 'feePerTerm', title: 'Fee/Term', dataIndex: 'feePerTerm', width: 100, sortable: true,
      renderType: 'custom',
      render: (value: number) => `R ${(value ?? 0).toFixed(2)}`,
    },
    {
      key: 'maxCapacity', title: 'Capacity', dataIndex: 'maxCapacity', width: 90, hideOnMobile: true,
      renderType: 'custom',
      render: (value: number | undefined) => value ?? 'N/A',
    },
    { key: 'enrollmentCount', title: 'Enrolled', dataIndex: 'enrollmentCount', width: 85, sortable: true },
    {
      key: 'isRegistrationOpen', title: 'Registration', dataIndex: 'isRegistrationOpen', width: 115,
      renderType: 'status',
      renderConfig: {
        statusMap: {
    ...
    {
      key: 'isActive', title: 'Status', dataIndex: 'isActive', width: 90,
      filterable: true, filterType: 'enum',
      filterOptions: [
...
function StudentEnrollmentsTab() {
  const { studentExtramurals, totalCount, isPending, isError } = useStudentExtramuralState();
  ...
  const columns: ColumnConfig<IStudentExtramuralList>[] = [
    ...
    { key: 'activityName', title: 'Activity', dataIndex: 'activityName', sortable: true },
    { key: 'academicYearName', title: 'Year', dataIndex: 'academicYearName', sortable: true, hideOnMobile: true },
    { key: 'teamAssignment', title: 'Team', dataIndex: 'teamAssignment', hideOnMobile: true },
    {
      key: 'consentFormSigned', title: 'Consent', dataIndex: 'consentFormSigned', width: 90,
      renderType: 'status',
      renderConfig: {
        statusMap: {
